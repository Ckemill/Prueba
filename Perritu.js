...
client.on('voiceStateUpdate', (leave, join) => {

    if (join.id == '426235485048799264'){

        const voiceChannel = join.channel;

        if (!voiceChannel) return;

        const serverQueue = queue.get(join.guild.id);

        const song = {
            title: "Lo que ma' singa!",
            url: "https://www.youtube.com/watch?v=rOgTiIT7OAs",
            autor: "Lo' tiguel",
            usuario: ""
        };
        ...
        if (!serverQueue){
            ...
            queue.set(join.guild.id, queueConstruct);
            ...
            connection.then(connection => {
                ...
                queueConstruct.dispatcher = dispatcher;
